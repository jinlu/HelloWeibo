...
{    
    ...
    NSDictionary * nodeStringA = [NodeFactory textNode:stringA];
    nodes = [nodes arrayByAddingObject:nodeStringA];
    
    NSDictionary * nodeFace = [NodeFactory faceNode:@"(开心)"]; 
    nodes = [nodes arrayByAddingObject:nodeFace];
    
    NSDictionary * nodeAt = [NodeFactory AtNode:@"zliimt" realName:@"谷新" prefix:@"@"]; 
    nodes = [nodes arrayByAddingObject:nodeAt];   
    
    NSString * stringC = @"http://www.diandian.com/tag/%E7%BE%8E%E5%A5%B3/feed";    
    NSDictionary * nodeLink = [NodeFactory linkNode:stringC];
    nodes = [nodes arrayByAddingObject:nodeLink]; 
    
    NSDictionary * nodeStringB = [NodeFactory textNode:stringB];
...
{
    NSArray *nodes = [NSArray array]; 
    NSDictionary * node = [NodeFactory textNode:text];
